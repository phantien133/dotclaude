...
| Work | Agent |
|---|---|
| Inventorying the project's actual tokens, component library and Code Connect state | `figma-design-system-mapper` |
| Pulling the Figma variable definitions the rules must describe | `figma-context-extractor` |

Generate rules from what the repo actually contains, not from the framework's defaults.
A rules file that describes a token scale the project does not have teaches the model to
invent values — the opposite of the point.

Cross-check the result against the `figma-fidelity` rules already installed by this
plugin; do not restate or contradict them, extend them with project specifics.
