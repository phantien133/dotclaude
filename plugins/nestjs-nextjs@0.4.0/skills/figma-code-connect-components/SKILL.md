...
| Work | Agent |
|---|---|
| Fetching node metadata / design context / existing `get_code_connect_map` entries | `figma-context-extractor` |
| Locating the real code component for a node, and judging whether it covers every variant | `figma-design-system-mapper` |

The mapper's Components table is the input to your mapping decisions: reuse where it says
`reuse`, extend where it says `extend`, and only propose a new component where it says
`create`. A mapping that points at a component which does not cover the Figma variants is
worse than no mapping — it makes future implementations silently wrong.
