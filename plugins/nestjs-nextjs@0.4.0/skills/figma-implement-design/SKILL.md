...
## Agent delegation (dotclaude)

Steps 2–4, 5 and 7 run in subagents. Two reasons, both load-bearing:

- **Token**: a full `get_design_context` payload plus screenshots can consume most of a
  context window. If it lands in the main context, the implementation that follows is
  written from a compressed memory of the design instead of the design.
- **Quality**: each agent has one job and a strict output contract, so values arrive as
  exact numbers in a file rather than as prose you have to re-derive.

| Step | Agent | Writes |
|---|---|---|
| 2–3 (context, variables, screenshots) | `figma-context-extractor` | `figma-spec.md` + `figma-screens/` |
| 4 (assets) | `figma-asset-fetcher` | `figma-assets.md` + real files on disk |
| 5 (tokens & components) | `figma-design-system-mapper` | `figma-mapping.md` |
| 7 (validation) | `figma-visual-verifier`, via `figma-verify-parity` | `figma-parity-report.md` |

You — the main agent — do step 6: write the code, reading the three files above. Do not
call the Figma MCP tools yourself unless an agent reported a gap you must resolve with one
targeted call.

The `figma-fidelity` rules bind every step. In particular: no generated SVG, no icon-pack
substitution, no rounded spacing, no guessed colours.
...
### Steps 2–3 (dotclaude): delegate extraction

Invoke the `figma-context-extractor` agent with the file key, node ID and an output path
(`<task folder>/figma-spec.md` when there is a task, otherwise the repo root). It performs
the whole `get_metadata` → `get_design_context` → `get_variable_defs` → `get_screenshot` →
`get_code_connect_map` sequence described below, including the truncation handling, and
returns a spec path plus a gap list.

Read the returned spec. Do not re-run the MCP calls. If the gap list is non-empty, resolve
those gaps with the developer **before** writing code — a gap is a question, not a licence
to pick a value.

The upstream description of the raw calls follows, as the contract the agent implements.
...
**dotclaude:** invoke the `figma-asset-fetcher` agent with the spec path and the project's
asset root. It downloads every entry in the spec's asset table, verifies each file, and
writes `figma-assets.md` with the exact import specifier for each one.

Unresolved assets are a **blocking** result, not a licence to improvise. Show the
developer the unresolved rows with their Figma export links and wait. Code may be written
around a pending asset with a `TODO(figma-asset): <node-id>` marker — never with a
hand-written SVG, an icon-pack substitute, or a coloured placeholder box.
...
**dotclaude:** invoke the `figma-design-system-mapper` agent first. It reads the spec and
the repo, then writes `figma-mapping.md` with, per Figma value, the project token to use
and whether the match is `exact`, `near (<delta>)` or `none`.

Use the mapping as the substitution table while you write code. Its "No equivalent" table
is a decision list for the developer — resolve it before implementing those values. A
`near` match with a visible delta means the design wins; see `figma-fidelity` §4.
...
**dotclaude:** run the `figma-verify-parity` skill (which drives the
`figma-visual-verifier` agent). It renders the screen in a real browser, reads
`getComputedStyle` for every property the spec pins, diffs numerically, checks that each
asset resolves to a real file, and compares against the Figma reference screenshot.

The implementation is complete only on `PASS`. `FAIL` means fix the listed defects and
re-run. `NOT RUN` is not a pass — surface why and resolve it.

Reading your own code and concluding it matches is not validation. That is the exact
