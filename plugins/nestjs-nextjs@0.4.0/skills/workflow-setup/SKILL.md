...
This wizard writes config schema **`CURRENT_VERSION = 2`**. Read the existing file's
`version:` field (a missing `version:` means `1`) and branch:

- **File missing + no `--reset`:** proceed to Step 1 (fresh setup).
- **File exists, `version == 2`, no `--reset`:** display current config and say:
  > Config already exists (version 2). Run `/workflow-setup --reset` to reconfigure, or `/dev-task <title>` to start a task.
  Stop here.
- **File exists, `version < 2` (or no `version:`), no `--reset`:** offer auto-migration
  (see Step 0a) — preserve existing values, add the new `repos:` list, bump to version 2.
- **File exists, `version > 2`:** the config is newer than this wizard. Say it's version
  `<N>` and to update the plugin; do not rewrite. Stop.
- **`--reset` flag:** display current config, then proceed to Step 1 (existing values are
  pre-filled defaults — press Enter to keep). A `--reset` write also upgrades to version 2.

---

## Step 0a — Migrate older config (auto, with confirm)

*Only when an existing file has `version < 2`.* The v2 addition is the top-level
**`repos:`** list (per-repo default branch — Step 6a). Migration preserves every
existing value and only ADDS the new fields. No re-interview.

1. Detect repos: `git submodule status 2>/dev/null` (and `.gitmodules`). For the main
   workspace `.` and each submodule path, resolve the default branch:
   `git -C <path> symbolic-ref --short refs/remotes/origin/HEAD 2>/dev/null | sed 's#^origin/##'`
   (main workspace falls back to old `pr.default_branch`, then `main`).
2. Show a migration card listing only the additions (`version: 2` + the derived
   `repos:` list, noting that `main` vs `master` differences are expected), and ask:
   `Migrate now? [Y]es / [e]dit a branch / [n]o`.
3. **Y** → write the upgraded file (Step 7) with every existing field unchanged plus
   `version: 2` and `repos:`. **e** → fix a repo's branch, re-show. **n** → leave as-is.

If there are no submodules, `repos:` is the single entry
...
## Step 6 — Repos & PR settings

### 6a — Repos (per-repo default branch)

Detect submodules (`git submodule status`). Each repo carries its **own** default
branch — they need not agree (`main` vs `master` is fine).

- **Single repo:** ask `Default branch for PRs [main]: ___` →
  `repos: [{path: ".", default_branch: <value>, remote: origin}]`.
- **Submodules present:** show the detected `.` + each submodule with its resolved
  default branch, let the developer edit any, record the full `repos:` list.

### 6b — PR options (apply to every repo)
...
Record `pr.draft`, `pr.template` (null if blank). Keep `pr.default_branch` = the
main workspace `.` default branch (fallback for older consumers).
...
```
✓ .claude/workflow.yaml written (version 2)

Repos:          . → main  [, apps/api → master, ...]
...
Lint:           npx eslint .
PR:             draft, one PR per repo against its own default branch
