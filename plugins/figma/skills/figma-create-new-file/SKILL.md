...
| Work | Agent |
|---|---|
| Reading existing file structure, variables, components and styles before generating | `figma-context-extractor` |
| Checking what the codebase already defines, when generating from an existing implementation | `figma-design-system-mapper` |

Inspect before you create. Generating a component or token that duplicates one already in
the file is the most common failure here, and it is only visible after the fact.

Keep the large MCP payloads inside the agent — this skill's own work is generation, and it
needs the context budget.
