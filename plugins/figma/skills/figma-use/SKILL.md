...
`use_figma` writes to a real Figma file — mistakes are visible to designers immediately, so
read before you write.

| Work | Agent |
|---|---|
| Reading the current state of the nodes you are about to modify | `figma-context-extractor` |
| Finding which existing variables / components to bind to instead of creating duplicates | `figma-design-system-mapper` |

Run the extractor before any destructive or structural edit, and again afterwards if you
need to confirm the result. Keep the raw payloads in the agent; work from its spec file.

The `figma-fidelity` rules apply in reverse here: do not invent variable values or
component variants to make a write succeed. If a binding target is missing, say so.
