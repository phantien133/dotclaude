...
5. **`plugins/`** — preset → plugin bundle build artifacts, published to self-hosted marketplace (`phantien133/claudekit-marketplace`).
...
pnpm publish-plugin <preset-name>
  ...
  ↓ (manual) copy plugins/<name>/ to phantien133/claudekit-marketplace + push
```
