...
Dev workflow preset for the Hilab streaming platform v3 (NestJS + Next.js + SRS + GraphQL).
Successor to `hilab-streaming-v2`: replaces the legacy `build-ui-kit` / `build-static-page` /
...
```bash
pnpm install:project hilab-streaming-v3 --force          # copy mode, overwrite without .bak
# or
pnpm install:project hilab-streaming-v3 --force --symlink # symlink mode for in-place edits
```
